// ==UserScript==
// @name         东方财富超大单与上证指数双轴走势图 (大屏版)
// @namespace    http://tampermonkey.net/
// @version      2.1
// @description  引入Chart.js，在右下角生成大尺寸的超大单净流入与上证指数双轴走势悬浮窗
// @author       Gemini
// @match        *://data.eastmoney.com/zjlx/dpzjlx.html*
// @updateURL    https://raw.githubusercontent.com/macrochen/my-scripts/main/userscripts/eastmoney-super-order-chart.user.js
// @downloadURL  https://raw.githubusercontent.com/macrochen/my-scripts/main/userscripts/eastmoney-super-order-chart.user.js
// @grant        none
...
(function() {
    ...
    const checkTimer = setInterval(() => {
        if (document.getElementById('em-super-order-chart-container')) {
    ...
    function initDashboard(rows) {
        ...
        rows.forEach(row => {
            ...
            if (cols.length >= 8) {
                ...
                const inflowText = cols[7].innerText.trim();
        ...
        container.id = 'em-super-order-chart-container';
        ...
        titleText.innerText = '超大单净流入 vs 上证指数';
        ...
        new Chart(canvas, {
            ...
            options: {
                ...
                scales: {
                    x: {
                        grid: { display: false },
                        ticks: { font: { size: 9 }, maxTicksLimit: 20, maxRotation: 0 } // 增加横轴标签显示数量
                    },
                    y: {
                        type: 'linear',
                        display: true,
