// ==UserScript==
// @name         创业板超大单与指数双轴走势图 (大屏版)
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  引入Chart.js，在创业板资金页面右下角生成大尺寸双轴走势悬浮窗
// @author       Gemini
// @match        *://data.eastmoney.com/zjlx/zs399006.html*
// @updateURL    https://raw.githubusercontent.com/macrochen/my-scripts/main/userscripts/chinext-super-order-chart.user.js
// @downloadURL  https://raw.githubusercontent.com/macrochen/my-scripts/main/userscripts/chinext-super-order-chart.user.js
// @grant        none
...
(function() {
    ...
    const checkTimer = setInterval(() => {
        if (document.getElementById('chinext-super-order-chart-container')) {
    ...
    function initDashboard(rows) {
        ...
        rows.forEach(row => {
            ...
            // 创业板页面表格列数较少，第2列（索引1）为收盘价，第6列（索引5）为超大单净流入净额
            if (cols.length >= 6) {
                const dateText = cols[0].innerText.trim();
                const indexText = cols[1].innerText.trim();
                const inflowText = cols[5].innerText.trim();
        ...
        container.id = 'chinext-super-order-chart-container';
        ...
        titleText.innerText = '超大单净流入 vs 创业板指';
